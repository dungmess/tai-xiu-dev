...
export const generateBetCommentary = async (result: string, total: number, dice: number[], isWin: boolean, amount: number): Promise<string> => {
  try {
    // Kiểm tra an toàn trước khi truy cập process.env
    const apiKey = typeof process !== 'undefined' && process.env ? process.env.API_KEY : null;
    
    if (!apiKey) {
      console.warn("API_KEY không tồn tại. Sử dụng bình luận mặc định.");
      return isWin ? "Thắng lớn rồi!" : "Thua rồi, đừng nản!";
    }
